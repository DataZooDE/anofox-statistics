...
did_estimate AS (
    SELECT
        (ols_fit_agg(sales, [treatment_post], {'intercept': true})).coefficients[1] as did_coefficient,
        (ols_fit_agg(sales, [treatment_post], {'intercept': true})).residual_standard_error as std_error,
        (ols_fit_agg(sales, [treatment_post], {'intercept': true})).r_squared as r2,
        COUNT(*) as n_obs
    FROM store_data
),
