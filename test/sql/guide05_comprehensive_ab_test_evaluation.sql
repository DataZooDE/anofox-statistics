...
conversion_test AS (
    SELECT
        (ols_fit_agg(conversion, [treatment], {'intercept': true})).coefficients[1] as treatment_effect,
        (ols_fit_agg(conversion, [treatment], {'intercept': true})).residual_standard_error as std_error,
        (ols_fit_agg(conversion, [treatment], {'intercept': true})).r_squared as r2,
        COUNT(*) as n_obs
    FROM experiment_data
),
...
revenue_test AS (
    SELECT
        (ols_fit_agg(revenue, [treatment], {'intercept': true})).coefficients[1] as treatment_effect,
        (ols_fit_agg(revenue, [treatment], {'intercept': true})).residual_standard_error as std_error,
        (ols_fit_agg(revenue, [treatment], {'intercept': true})).r_squared as r2,
        COUNT(*) as n_obs
    FROM experiment_data
),
