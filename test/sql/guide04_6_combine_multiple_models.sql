...
WITH simple_model AS (
    SELECT
        ...
        ROUND((ols_fit_agg(y, [x1], {'intercept': true})).r_squared, 4) as r2,
        COUNT(*) as n_obs,
        1 as n_predictors
...
complex_predictors AS (
    SELECT 'x1' as var, (ols_fit_agg(y, [x1], {'intercept': true})).r_squared as r2 FROM model_comparison_data
    UNION ALL
    SELECT 'x2' as var, (ols_fit_agg(y, [x2], {'intercept': true})).r_squared as r2 FROM model_comparison_data
    UNION ALL
    SELECT 'x3' as var, (ols_fit_agg(y, [x3], {'intercept': true})).r_squared as r2 FROM model_comparison_data
    UNION ALL
    SELECT 'x4' as var, (ols_fit_agg(y, [x4], {'intercept': true})).r_squared as r2 FROM model_comparison_data
),
complex_summary AS (
    SELECT
