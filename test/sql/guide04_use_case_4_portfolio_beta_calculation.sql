...
SELECT
    stock_ticker,
    ROUND((ols_fit_agg(stock_return, [market_return], {'intercept': true})).coefficients[1], 3) as beta,
    ROUND((ols_fit_agg(stock_return, [market_return], {'intercept': true})).r_squared, 3) as r2,
    CASE
        WHEN (ols_fit_agg(stock_return, [market_return], {'intercept': true})).coefficients[1] > 1.2 THEN 'High Risk'
        WHEN (ols_fit_agg(stock_return, [market_return], {'intercept': true})).coefficients[1] > 0.8 THEN 'Medium Risk'
        ELSE 'Low Risk'
    END as risk_category,
    CASE
        WHEN (ols_fit_agg(stock_return, [market_return], {'intercept': true})).coefficients[1] > 1.0 THEN 'Aggressive'
        WHEN (ols_fit_agg(stock_return, [market_return], {'intercept': true})).coefficients[1] > 0.5 THEN 'Moderate'
        ELSE 'Defensive'
