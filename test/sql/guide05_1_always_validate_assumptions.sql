...
WITH validation AS (
    ...
    FROM vif([[1.0::DOUBLE, 2.0, 3.0], [1.1::DOUBLE, 2.1, 3.1], [1.2::DOUBLE, 2.2, 3.2], [1.3::DOUBLE, 2.3, 3.3], [1.4::DOUBLE, 2.4, 3.4]])
    UNION ALL
    SELECT
        'Normality',
        CAST(p_value AS VARCHAR),
        CASE WHEN p_value > 0.05 THEN 'PASS' ELSE 'FAIL' END
    FROM normality_test([0.1::DOUBLE, -0.2, 0.3, -0.1, 0.2, -0.3, 0.15, -0.25], 0.05)
    UNION ALL
    SELECT
        'Outliers',
