...
-- Tests: anofox_statistics_ols_agg, anofox_statistics_wls_agg,
--        anofox_statistics_ridge_agg, anofox_statistics_rls_agg
--
...
FROM (
    SELECT
        category,
        anofox_statistics_ols_agg(y, [x1, x2], {'intercept': true}) as result
...
FROM (
    SELECT
        category,
        anofox_statistics_ols_agg(y, [x1, x2], {'intercept': false}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_wls_agg(y, [x1, x2], weight, {'intercept': true}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_wls_agg(y, [x1, x2], weight, {'intercept': false}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_ridge_agg(y, [x1, x2], {'lambda': 1.0, 'intercept': true}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_ridge_agg(y, [x1, x2], {'lambda': 1.0, 'intercept': false}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_rls_agg(y, [x1, x2], {'forgetting_factor': 1.0, 'intercept': true}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_rls_agg(y, [x1, x2], {'forgetting_factor': 0.95, 'intercept': false}) as result
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        category,
        anofox_statistics_ols_agg(y, [x1, x2], {'intercept': true}) as ols,
        anofox_statistics_wls_agg(y, [x1, x2], weight, {'intercept': true}) as wls,
        anofox_statistics_ridge_agg(y, [x1, x2], {'lambda': 0.1, 'intercept': true}) as ridge,
        anofox_statistics_rls_agg(y, [x1, x2], {'forgetting_factor': 1.0, 'intercept': true}) as rls
    FROM aggregate_test_data
    GROUP BY category
) sub;
...
FROM (
    SELECT
        anofox_statistics_ols_agg(y, [x1, x1, x1], {'intercept': true}) as result
    FROM small_data
) sub;
