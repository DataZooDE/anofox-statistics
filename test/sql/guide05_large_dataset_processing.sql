...
monthly_models AS (
    SELECT
        ...
        (SELECT (ols_fit_agg(y, [x], {'intercept': true})).coefficients[1] FROM (
            SELECT UNNEST(y_values) as y, UNNEST(x_values) as x
        )) as coefficient,
        (SELECT (ols_fit_agg(y, [x], {'intercept': true})).r_squared FROM (
            SELECT UNNEST(y_values) as y, UNNEST(x_values) as x
        )) as r2
...
sample_model AS (
    SELECT
        (ols_fit_agg(y, [x1], {'intercept': true})).coefficients[1] as coeff_x1,
        (ols_fit_agg(y, [x2], {'intercept': true})).coefficients[1] as coeff_x2,
        (ols_fit_agg(y, [x1], {'intercept': true})).r_squared as r2_x1,
        (ols_fit_agg(y, [x2], {'intercept': true})).r_squared as r2_x2
    FROM sample_data
)
