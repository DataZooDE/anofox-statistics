...
SELECT
    department,
    ROUND((ols_fit_agg(output_per_hour, [training_hours], {'intercept': true})).coefficients[1], 2) as training_impact,
    ROUND((ols_fit_agg(output_per_hour, [training_hours], {'intercept': true})).r_squared, 3) as model_fit,
    CASE
        WHEN (ols_fit_agg(output_per_hour, [training_hours], {'intercept': true})).coefficients[1] > 5.0 THEN 'High Training ROI'
        WHEN (ols_fit_agg(output_per_hour, [training_hours], {'intercept': true})).coefficients[1] > 2.0 THEN 'Medium Training ROI'
        ELSE 'Low Training ROI'
    END as training_effectiveness,
    CASE
        WHEN (ols_fit_agg(output_per_hour, [training_hours], {'intercept': true})).coefficients[1] > 3.0 THEN 'Increase Training Budget'
