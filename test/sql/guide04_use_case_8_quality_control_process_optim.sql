...
parameter_impacts AS (
    SELECT
        ...
        ROUND((ols_fit_agg(defect_rate, [temperature], {'intercept': true})).coefficients[1], 4) as impact_on_defects,
        ROUND((ols_fit_agg(defect_rate, [temperature], {'intercept': true})).r_squared, 3) as model_fit
    FROM recent_batches
    UNION ALL
    SELECT
        'Pressure' as variable,
        ROUND((ols_fit_agg(defect_rate, [pressure], {'intercept': true})).coefficients[1], 4) as impact_on_defects,
        ROUND((ols_fit_agg(defect_rate, [pressure], {'intercept': true})).r_squared, 3) as model_fit
    FROM recent_batches
    UNION ALL
    SELECT
        'Humidity' as variable,
        ROUND((ols_fit_agg(defect_rate, [humidity], {'intercept': true})).coefficients[1], 4) as impact_on_defects,
        ROUND((ols_fit_agg(defect_rate, [humidity], {'intercept': true})).r_squared, 3) as model_fit
    FROM recent_batches
    UNION ALL
    SELECT
        'Line Speed' as variable,
        ROUND((ols_fit_agg(defect_rate, [line_speed], {'intercept': true})).coefficients[1], 4) as impact_on_defects,
        ROUND((ols_fit_agg(defect_rate, [line_speed], {'intercept': true})).r_squared, 3) as model_fit
    FROM recent_batches
),
