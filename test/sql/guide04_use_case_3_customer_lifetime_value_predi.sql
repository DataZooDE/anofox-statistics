...
SELECT
    ...
    ROUND((ols_fit_agg(total_purchases, [tenure], {'intercept': true})).coefficients[1], 2) as value
FROM customer_summary
WHERE cohort_month <= CURRENT_DATE - INTERVAL '12 months'
UNION ALL
SELECT
    'Model Coefficient: AOV' as metric,
    ROUND((ols_fit_agg(total_purchases, [aov], {'intercept': true})).coefficients[1], 2) as value
FROM customer_summary
WHERE cohort_month <= CURRENT_DATE - INTERVAL '12 months'
UNION ALL
SELECT
    'Model Coefficient: Frequency' as metric,
    ROUND((ols_fit_agg(total_purchases, [freq], {'intercept': true})).coefficients[1], 2) as value
FROM customer_summary
WHERE cohort_month <= CURRENT_DATE - INTERVAL '12 months'
UNION ALL
SELECT
    'Model Quality (R²)' as metric,
    ROUND((ols_fit_agg(total_purchases, [tenure], {'intercept': true})).r_squared, 3) as value
FROM customer_summary
WHERE cohort_month <= CURRENT_DATE - INTERVAL '12 months';
