 ...
 * Used for computing p-values, confidence intervals, and critical values
...
inline double student_t_critical(double alpha, int df) {
	...
	if (df > 30) {
		...
		// Check from largest to smallest alpha to match correctly
		if (alpha >= 0.10)
			return 1.697; // 90%
		if (alpha >= 0.05)
			return 2.042; // 95%
		...
		if (alpha >= 0.001)
			return 3.291; // 99.9%
		return 3.291;     // For very small alpha
	} else {
...
public:
	...
	static double ComplementaryCDF(double x, double df) {
		...
		// where P(gamma(a, x)) is the regularized lower incomplete gamma function

		// Use relationship: P(X ≤ x) = P(Gamma(df/2, x/2))
		...
		// Compute using incomplete gamma function
		// P(X ≤ x) = gamma_inc_reg(df/2, x/2)
		double a = df / 2.0;
		double z = x / 2.0;
