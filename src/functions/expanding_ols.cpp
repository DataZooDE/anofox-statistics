 ...
 *
 * Computes OLS regression on expanding/cumulative windows of observations.
 * For data with n observations and min_periods m:
 *   - Window 0: observations [0, m)
 *   - Window 1: observations [0, m+1)
 *   - ...
 *   - Window (n-m): observations [0, n)
 *
 * All windows start at index 0 and expand to include more observations.
...
static void ComputeExpandingOLS(ExpandingOlsBindData &data) {
	...
	if (m < p + 1) {
		throw InvalidInputException("Minimum periods must be at least %d (n_features + 1), got %d", p + 1, m);
	}

	if (m > n) {
		throw InvalidInputException("Minimum periods (%d) cannot exceed number of observations (%d)", m, n);
	}

	ANOFOX_DEBUG("Computing expanding OLS with " << n << " observations, " << p << " features, min_periods = " << m);

	// Compute OLS for each expanding window
	// Windows: [0, m), [0, m+1), ..., [0, n)
	idx_t n_windows = n - m + 1;
	...
	ANOFOX_INFO("Expanding OLS complete: computed " << n_windows << " windows");
}

static unique_ptr<FunctionData> ExpandingOlsBind(ClientContext &context, TableFunctionBindInput &input,
                                                 vector<LogicalType> &return_types, vector<string> &names) {

	ANOFOX_INFO("Expanding OLS bind phase");
	...
	ANOFOX_INFO("Expanding OLS completed: " << result->windows.size() << " windows");
...
void ExpandingOlsFunction::Register(ExtensionLoader &loader) {
	...
	ANOFOX_DEBUG("anofox_statistics_expanding_ols registered successfully");
}
