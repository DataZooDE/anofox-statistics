...
/**
 * Bind data for Ridge inference table function
 */
struct RidgeInferenceBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<RidgeInferenceBindData>();
...
/**
 * Bind function - Extract arrays and compute Ridge inference
 */
static unique_ptr<FunctionData> RidgeInferenceBind(ClientContext &context, TableFunctionBindInput &input,
                                                    vector<LogicalType> &return_types, vector<string> &names) {

	auto bind_data = make_uniq<RidgeInferenceBindData>();

	// Get parameters
	auto &y_value = input.inputs[0];
	auto &x_value = input.inputs[1];

	// Parse options from third parameter if provided
	RegressionOptions options;
	if (input.inputs.size() > 2) {
		options = RegressionOptions::ParseFromMap(input.inputs[2]);
	}

	// Ridge requires lambda
	if (options.lambda <= 0.0) {
		throw InvalidInputException("Ridge inference requires lambda > 0 (got lambda=%f)", options.lambda);
	}
	...
	// Fit Ridge regression with full output to get standard errors
	auto ridge_result = bridge::LibanostatWrapper::FitRidge(y_values, x_matrix, options, true);

	// Extract results
	Eigen::VectorXd coefficients = ridge_result.coefficients;
	double intercept = ridge_result.intercept;
	idx_t rank = ridge_result.rank;
	idx_t df = n - rank;
	...
	// Extract MSE
	double mse = ridge_result.mse;

	// Compute critical value for confidence intervals
	double alpha = 1.0 - confidence_level;
	double t_crit = student_t_critical(alpha / 2.0, static_cast<int>(df));

	// Ridge standard errors (accounting for bias from regularization)
	Eigen::VectorXd std_errors_vec;
	if (ridge_result.has_std_errors) {
		std_errors_vec = ridge_result.std_errors;
	} else {
		// Fallback: compute from (X'X + λI)^(-1) if not provided
		// For now, set to NaN if not available
		std_errors_vec = Eigen::VectorXd::Constant(coefficients.size(), std::numeric_limits<double>::quiet_NaN());
	}

	// If add_intercept, process intercept first
	if (add_intercept) {
		double intercept_se = std::numeric_limits<double>::quiet_NaN();

		// Extract intercept std error if available
		if (ridge_result.has_intercept && ridge_result.has_std_errors && std::isfinite(ridge_result.intercept_std_error)) {
			intercept_se = ridge_result.intercept_std_error;
		} else if (ridge_result.has_std_errors && std_errors_vec.size() > p) {
			// Intercept SE might be last element
		...
		ANOFOX_DEBUG("Ridge Inference: intercept = " << intercept << " (SE=" << intercept_se << ", t=" << t_stat
		                                              << ", p=" << p_value << ")");
	}

	// Store results for each slope coefficient
	for (idx_t j = 0; j < p; j++) {
		string var_name = "x" + std::to_string(j + 1);

		double estimate = coefficients(j);
		bool is_aliased = ridge_result.is_aliased[j];

		double std_error, t_stat, p_value, ci_lower, ci_upper;
		bool is_sig;

		if (is_aliased || std::isnan(estimate)) {
			// Aliased coefficient -> all statistics are NaN
			...
			ANOFOX_DEBUG("Ridge Inference: " << var_name << " = ALIASED (NULL)");
		} else {
			// Valid coefficient - use Ridge-adjusted standard errors
			...
			ANOFOX_DEBUG("Ridge Inference: " << var_name << " = " << estimate << " (SE=" << std_error << ", t=" << t_stat
			                                  << ", p=" << p_value << ", lambda=" << options.lambda << ")");
		}
 ...
 */
static void RidgeInferenceTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<RidgeInferenceBindData>();
...
void RidgeInferenceFunction::Register(ExtensionLoader &loader) {
	ANOFOX_DEBUG("Registering Ridge inference function");

	TableFunction ridge_inference_func("anofox_statistics_ridge_inference",
	                                    {LogicalType::LIST(LogicalType::DOUBLE),                    // y
	                                     LogicalType::LIST(LogicalType::LIST(LogicalType::DOUBLE))}, // X (matrix)
	                                    RidgeInferenceTableFunc, RidgeInferenceBind);

	// Add support for optional MAP parameter via varargs
	ridge_inference_func.varargs = LogicalType::ANY;

	loader.RegisterFunction(ridge_inference_func);

	ANOFOX_DEBUG("Ridge inference function registered successfully");
}
