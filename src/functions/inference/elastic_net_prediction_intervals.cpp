#include "elastic_net_prediction_intervals.hpp"
...
struct ElasticNetPredictIntervalBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<ElasticNetPredictIntervalBindData>();
...
static unique_ptr<FunctionData> ElasticNetPredictIntervalBind(ClientContext &context, TableFunctionBindInput &input,
	...
	// Fit Elastic Net model with full output
	auto result = bridge::LibanostatWrapper::FitElasticNet(y_train, x_train_matrix, options, true);
	...
	for (idx_t i = 0; i < n_new; i++) {
		...
		ANOFOX_DEBUG("Elastic Net Prediction " << i << ": " << y_pred << " [" << ci_lower << ", " << ci_upper << "]");
	}
...
static void ElasticNetPredictIntervalTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<ElasticNetPredictIntervalBindData>();
