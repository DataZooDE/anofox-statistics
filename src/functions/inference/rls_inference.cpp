...
struct RlsInferenceBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<RlsInferenceBindData>();
...
static unique_ptr<FunctionData> RlsInferenceBind(ClientContext &context, TableFunctionBindInput &input,
                                                  vector<LogicalType> &return_types, vector<string> &names) {

	auto bind_data = make_uniq<RlsInferenceBindData>();
	...
	// Fit RLS with full output
	auto rls_result = bridge::LibanostatWrapper::FitRLS(y_values, x_matrix, options, true);

	Eigen::VectorXd coefficients = rls_result.coefficients;
	double intercept = rls_result.intercept;
	idx_t rank = rls_result.rank;
	idx_t df = n - rank;
	...
	if (add_intercept) {
		...
		if (rls_result.has_intercept && rls_result.has_std_errors && std::isfinite(rls_result.intercept_std_error)) {
			intercept_se = rls_result.intercept_std_error;
		} else if (rls_result.has_std_errors && std_errors_vec.size() > p) {
		...
		ANOFOX_DEBUG("RLS Inference: intercept = " << intercept << " (SE=" << intercept_se << ")");
	}
	...
	for (idx_t j = 0; j < p; j++) {
		...
		bool is_aliased = rls_result.is_aliased[j];
...
static void RlsInferenceTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<RlsInferenceBindData>();
...
void RlsInferenceFunction::Register(ExtensionLoader &loader) {
	ANOFOX_DEBUG("Registering RLS inference function");

	TableFunction rls_inference_func("anofox_statistics_rls_inference",
	                                  {LogicalType::LIST(LogicalType::DOUBLE),                    // y
	                                   LogicalType::LIST(LogicalType::LIST(LogicalType::DOUBLE))}, // X
	                                  RlsInferenceTableFunc, RlsInferenceBind);

	// Add support for optional MAP parameter via varargs
	rls_inference_func.varargs = LogicalType::ANY;

	loader.RegisterFunction(rls_inference_func);

	ANOFOX_DEBUG("RLS inference function registered successfully");
}
