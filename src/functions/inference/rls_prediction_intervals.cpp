#include "rls_prediction_intervals.hpp"
...
struct RlsPredictIntervalBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<RlsPredictIntervalBindData>();
...
static unique_ptr<FunctionData> RlsPredictIntervalBind(ClientContext &context, TableFunctionBindInput &input,
	...
	// Fit RLS model with full output
	auto result = bridge::LibanostatWrapper::FitRLS(y_train, x_train_matrix, options, true);
	...
	for (idx_t i = 0; i < n_new; i++) {
		...
		ANOFOX_DEBUG("RLS Prediction " << i << ": " << y_pred << " [" << ci_lower << ", " << ci_upper << "]");
	}
...
static void RlsPredictIntervalTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<RlsPredictIntervalBindData>();
...
void RLSPredictIntervalFunction::Register(ExtensionLoader &loader) {
	...
	TableFunction func("anofox_stats_predict_rls",
	...
	ANOFOX_DEBUG("RLS predict_interval function registered successfully (including alias predict_rls)");
}
