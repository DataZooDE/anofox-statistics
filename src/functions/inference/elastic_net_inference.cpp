#include "elastic_net_inference.hpp"
...
struct ElasticNetInferenceBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<ElasticNetInferenceBindData>();
...
static unique_ptr<FunctionData> ElasticNetInferenceBind(ClientContext &context, TableFunctionBindInput &input,
                                                          vector<LogicalType> &return_types, vector<string> &names) {

	auto bind_data = make_uniq<ElasticNetInferenceBindData>();
	...
	// Fit Elastic Net with full output
	auto enet_result = bridge::LibanostatWrapper::FitElasticNet(y_values, x_matrix, options, true);

	Eigen::VectorXd coefficients = enet_result.coefficients;
	double intercept = enet_result.intercept;
	idx_t rank = enet_result.rank;
	idx_t df = n - rank;
	...
	if (add_intercept) {
		...
		if (enet_result.has_intercept && enet_result.has_std_errors && std::isfinite(enet_result.intercept_std_error)) {
			intercept_se = enet_result.intercept_std_error;
		} else if (enet_result.has_std_errors && std_errors_vec.size() > p) {
		...
		ANOFOX_DEBUG("Elastic Net Inference: intercept = " << intercept << " (SE=" << intercept_se << ")");
	}

	// Process coefficients (handle sparse solutions)
	for (idx_t j = 0; j < p; j++) {
		string var_name = "x" + std::to_string(j + 1);
		double estimate = coefficients(j);
		bool is_aliased = enet_result.is_aliased[j];
		bool is_zero = (std::abs(estimate) < 1e-10); // Elastic Net can produce exact zeros

		double std_error, t_stat, p_value, ci_lower, ci_upper;
		bool is_sig;

		if (is_aliased || std::isnan(estimate) || is_zero) {
			// Zero coefficient from L1 penalty -> set to NULL
			estimate = is_zero ? 0.0 : std::numeric_limits<double>::quiet_NaN();
			...
			ANOFOX_DEBUG("Elastic Net Inference: " << var_name << " = " << (is_zero ? "ZERO (sparse)" : "ALIASED"));
		} else {
			...
			ANOFOX_DEBUG("Elastic Net Inference: " << var_name << " = " << estimate << " (SE=" << std_error << ")");
		}
...
static void ElasticNetInferenceTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<ElasticNetInferenceBindData>();
...
void ElasticNetInferenceFunction::Register(ExtensionLoader &loader) {
	ANOFOX_DEBUG("Registering Elastic Net inference function");

	TableFunction enet_inference_func("anofox_statistics_elastic_net_inference",
	                                   {LogicalType::LIST(LogicalType::DOUBLE),                    // y
	                                    LogicalType::LIST(LogicalType::LIST(LogicalType::DOUBLE))}, // X
	                                   ElasticNetInferenceTableFunc, ElasticNetInferenceBind);

	// Add support for optional MAP parameter via varargs
	enet_inference_func.varargs = LogicalType::ANY;

	loader.RegisterFunction(enet_inference_func);

	ANOFOX_DEBUG("Elastic Net inference function registered successfully");
}
