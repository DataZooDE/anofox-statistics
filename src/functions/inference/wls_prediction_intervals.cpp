#include "wls_prediction_intervals.hpp"
...
struct WlsPredictIntervalBindData : public FunctionData {
	...
	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<WlsPredictIntervalBindData>();
...
static unique_ptr<FunctionData> WlsPredictIntervalBind(ClientContext &context, TableFunctionBindInput &input,
	...
	auto &x_new_value = input.inputs[3];
	auto &options_value = input.inputs[4];
	...
	// Fit WLS model with full output
	auto result = bridge::LibanostatWrapper::FitWLS(y_train, x_train_matrix, weights, options, true);
	...
	for (idx_t i = 0; i < n_new; i++) {
		...
		ANOFOX_DEBUG("WLS Prediction " << i << ": " << y_pred << " [" << ci_lower << ", " << ci_upper << "]");
	}
...
static void WlsPredictIntervalTableFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &bind_data = data_p.bind_data->CastNoConst<WlsPredictIntervalBindData>();
