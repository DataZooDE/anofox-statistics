...
// State for RLS fit-predict (empty - window-only approach)
 ...
 */
static void RlsFitPredictWindow(duckdb::AggregateInputData &aggr_input_data, const duckdb::WindowPartitionInput &partition,
    ...
    ANOFOX_DEBUG("RLS fit-predict: n_train=" << n_train << ", forgetting_factor=" << options.forgetting_factor << ", yhat=" << pred.yhat);
}
...
void RlsFitPredictFunction::Register(ExtensionLoader &loader) {
    ...
    AggregateFunction anofox_statistics_fit_predict_rls(
        ...
        AggregateFunction::StateSize<RlsFitPredictState>,
