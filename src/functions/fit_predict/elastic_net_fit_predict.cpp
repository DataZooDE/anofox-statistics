...
// State for Elastic Net fit-predict (empty - window-only approach)
 ...
 */
static void ElasticNetFitPredictWindow(duckdb::AggregateInputData &aggr_input_data, const duckdb::WindowPartitionInput &partition,
    ...
    for (const auto &frame : subframes) {
        for (idx_t frame_idx = frame.start; frame_idx < frame.end; frame_idx++) {
            if (frame_idx < all_y.size() &&
                !std::isnan(all_y[frame_idx]) &&
                !all_x[frame_idx].empty()) {
    ...
    if (options.intercept) {
        ...
        auto result = ElasticNetSolver::Fit(y_centered, X_centered,
                                            options.alpha,
                                            options.lambda);
        ...
        // No intercept
        auto result = ElasticNetSolver::Fit(y_train, X_train,
                                            options.alpha,
                                            options.lambda);
        beta = result.coefficients;
        rank = (beta.array().abs() > 1e-10).count();  // Count non-zero coefficients
    ...
    PredictionResult pred = ComputePredictionWithInterval(
        all_x[rid],
        intercept,
        beta,
        mse,
    ...
    ANOFOX_DEBUG("Elastic Net fit-predict: n_train=" << n_train << ", alpha=" << options.alpha << ", lambda=" << options.lambda << ", yhat=" << pred.yhat);
}
...
void ElasticNetFitPredictFunction::Register(ExtensionLoader &loader) {
    ...
    AggregateFunction anofox_statistics_fit_predict_elastic_net(
        ...
        AggregateFunction::StateSize<ElasticNetFitPredictState>,
