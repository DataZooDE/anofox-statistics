...
void OlsFitPredictCombine(Vector &source, Vector &target, AggregateInputData &aggr_input_data, idx_t count) {
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        // Merge training data
        target_state.y_train.insert(target_state.y_train.end(),
                                     source_state.y_train.begin(),
                                     source_state.y_train.end());
        target_state.x_train.insert(target_state.x_train.end(),
                                     source_state.x_train.begin(),
                                     source_state.x_train.end());

        // Merge all data
        target_state.x_all.insert(target_state.x_all.end(),
                                   source_state.x_all.begin(),
                                   source_state.x_all.end());
        target_state.is_train_row.insert(target_state.is_train_row.end(),
                                          source_state.is_train_row.begin(),
