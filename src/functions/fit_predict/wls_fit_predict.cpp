...
void WlsFitPredictFunction::Register(ExtensionLoader &loader) {
	...
	AggregateFunction anofox_stats_wls_fit_predict(
	...
	loader.RegisterFunction(anofox_stats_wls_fit_predict);

	// Register alias
	AggregateFunction wls_fit_predict(
	    "wls_fit_predict",
	    {LogicalType::DOUBLE, LogicalType::LIST(LogicalType::DOUBLE), LogicalType::DOUBLE, LogicalType::ANY},
	    LogicalType::STRUCT(fit_predict_struct_fields), AggregateFunction::StateSize<WlsFitPredictState>,
	    WlsFitPredictInitialize,
	...
	loader.RegisterFunction(wls_fit_predict);

	ANOFOX_DEBUG("WLS fit-predict function registered successfully (including alias wls_fit_predict)");
}
