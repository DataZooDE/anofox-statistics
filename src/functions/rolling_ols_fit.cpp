...
/**
 * @brief Bind data structure for rolling OLS fit function
 *
 * Stores rolling window state and parameters
 */
struct RollingOlsFitBindData : public FunctionData {
	// Input parameters
	...
	std::vector<Eigen::MatrixXd> window_x_data;
	std::vector<Eigen::VectorXd> window_y_data;
	std::vector<idx_t> window_indices;
	...
	RollingOlsFitBindData() : window_size(50), add_intercept(true), result_index(0) {
	}

	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<RollingOlsFitBindData>();
		...
		result->window_x_data = window_x_data;
		result->window_y_data = window_y_data;
...
unique_ptr<FunctionData> RollingOlsFitFunction::RollingOlsFitBind(ClientContext &context,
                                                                  TableFunctionBindInput &input) {
	ANOFOX_INFO("Starting rolling OLS fit bind phase");

	auto result = make_uniq<RollingOlsFitBindData>();

	// Parse input parameters
	if (input.inputs.size() < 4 || input.inputs.size() > 7) {
		throw InvalidInputException("anofox_statistics_rolling_ols_fit requires 4-7 parameters: "
		                            "data, x_cols, y_col, window_size, [partition_col], [order_col], [add_intercept]");
	}
	...
	if (input.inputs[3]->type != ExpressionType::VALUE_CONSTANT) {
		throw InvalidInputException("window_size must be a constant");
	}
	auto &window_size_const = dynamic_cast<BoundConstantExpression &>(*input.inputs[3]);
	if (window_size_const.value.type() != LogicalType::BIGINT) {
		throw InvalidInputException("window_size must be an INTEGER");
	}
	result->window_size = window_size_const.value.GetValue<int64_t>();
	if (result->window_size < 2) {
		throw InvalidInputException("window_size must be >= 2");
	}
	...
	ANOFOX_DEBUG("Rolling OLS parameters: window_size=" + std::to_string(result->window_size) + ", x_cols=" +
	             std::to_string(result->x_col_names.size()) + ", intercept=" + std::to_string(result->add_intercept));

	ANOFOX_INFO("Rolling OLS fit bind phase completed");

	return std::move(result);
}

void RollingOlsFitFunction::RollingOlsFitExecute(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	ANOFOX_DEBUG("Rolling OLS fit execute phase");

	auto &bind_data = dynamic_cast<RollingOlsFitBindData &>(*data.bind_data);
	...
	ANOFOX_DEBUG("Rolling OLS fit execute completed");
}

void RollingOlsFitFunction::Register(ExtensionLoader &loader) {
	ANOFOX_INFO("Registering anofox_statistics_rolling_ols_fit table function");

	auto rolling_ols_func = make_uniq<TableFunction>(
	    "anofox_statistics_rolling_ols_fit",
	    std::vector<LogicalType> {LogicalType::TABLE, LogicalType::LIST(LogicalType::VARCHAR), LogicalType::VARCHAR,
	                              ...
	                              LogicalType::BOOLEAN},
	    RollingOlsFitExecute, RollingOlsFitBind);

	rolling_ols_func->bind_replace_projection = true;

	loader.RegisterTableFunction(std::move(rolling_ols_func));

	ANOFOX_INFO("anofox_statistics_rolling_ols_fit registered successfully");
}
