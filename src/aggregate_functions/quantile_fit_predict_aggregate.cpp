...
static bool QuantileIsSplitTraining(const string_t &split_val) {
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        if (bind_data.use_split_col && split_values) {
            ...
            if (split_data.validity.RowIsValid(split_idx)) {
                row_is_training = QuantileIsSplitTraining(split_values[split_idx]);
            } else {
...
static unique_ptr<FunctionData> QuantilePredictAggBindWithSplit(ClientContext &context, AggregateFunction &function,
                                                                  vector<unique_ptr<Expression>> &arguments) {
    auto result = make_uniq<QuantilePredictAggBindData>();
