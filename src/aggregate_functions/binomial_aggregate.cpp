...
//===--------------------------------------------------------------------===//
// Binomial GLM aggregate. y is the success rate (0..1) per row; the user
// chooses one of logit / probit / cloglog as the link function (default
// logit).
//===--------------------------------------------------------------------===//
struct BinomialAggregateState {
    ...
    BinomialAggregateState()
        : n_features(0), initialized(false), fit_intercept(true), link(ANOFOX_BINOMIAL_LINK_LOGIT),
...
static AnofoxBinomialLink ConvertBinomialLink(BinomialLink link) {
    switch (link) {
    case BinomialLink::LOGIT:
        return ANOFOX_BINOMIAL_LINK_LOGIT;
    case BinomialLink::PROBIT:
        return ANOFOX_BINOMIAL_LINK_PROBIT;
    case BinomialLink::CLOGLOG:
        return ANOFOX_BINOMIAL_LINK_CLOGLOG;
    default:
        return ANOFOX_BINOMIAL_LINK_LOGIT;
    }
}

static LogicalType GetBinomialAggResultType(bool compute_inference) {
...
static void BinomialAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) BinomialAggregateState();
}

static void BinomialAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (BinomialAggregateState **)sdata.data;

    for (idx_t i = 0; i < count; i++) {
        auto &state = *states[sdata.sel->get_index(i)];
        state.~BinomialAggregateState();
    }
}

static void BinomialAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                              Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<BinomialAggregateBindData>();
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        state.link = ConvertBinomialLink(bind_data.link);
...
static void BinomialAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    ...
    auto sources = (BinomialAggregateState **)source_data.data;
    auto targets = (BinomialAggregateState **)target_data.data;
...
static void BinomialAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result, idx_t count,
                                idx_t offset) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (BinomialAggregateState **)sdata.data;
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        bool success = anofox_binomial_fit(y_array, x_arrays.data(), x_arrays.size(), options, &core_result,
        ...
        // For Binomial the dispersion is 1 by definition (canonical exponential
        // family). The field is populated by the upstream solver but typically
        // reads as 1.0 unless quasi-binomial dispersion estimation is enabled.
...
static unique_ptr<FunctionData> BinomialAggBind(ClientContext &context, AggregateFunction &function,
    ...
    if (arguments.size() >= 3 && arguments[2]->IsFoldable()) {
        ...
        if (opts.binomial_link.has_value()) {
            result->link = opts.binomial_link.value();
        }
    }
...
void RegisterBinomialAggregateFunction(ExtensionLoader &loader) {
    ...
    auto map_func = AggregateFunction(
        "anofox_stats_binomial_fit_agg",
        {LogicalType::DOUBLE, LogicalType::LIST(LogicalType::DOUBLE), LogicalType::ANY}, LogicalType::ANY,
        AggregateFunction::StateSize<BinomialAggregateState>, BinomialAggInitialize, BinomialAggUpdate,
        BinomialAggCombine, BinomialAggFinalize, nullptr, BinomialAggBind, BinomialAggDestroy);
