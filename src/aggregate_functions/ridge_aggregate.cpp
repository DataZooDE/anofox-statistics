...
//===--------------------------------------------------------------------===//
// Ridge Aggregate State - accumulates y and x values for each group
//===--------------------------------------------------------------------===//
struct RidgeAggregateState {
    ...
    RidgeAggregateState()
        : n_features(0), initialized(false), alpha(1.0), fit_intercept(true), compute_inference(false),
          confidence_level(0.95), solver(SolverType::SVD), lambda_scaling(LambdaScaling::RAW) {}
...
//===--------------------------------------------------------------------===//
struct RidgeAggregateBindData : public FunctionData {
    double alpha = 1.0;
...
//===--------------------------------------------------------------------===//
static LogicalType GetRidgeAggResultType(bool compute_inference) {
...
static void RidgeAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) RidgeAggregateState();
}

// Destroy aggregate state
static void RidgeAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        state.~RidgeAggregateState();
    }
}

// Update: accumulate values from input rows
static void RidgeAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                           Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<RidgeAggregateBindData>();
...
static void RidgeAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    ...
    auto sources = (RidgeAggregateState **)source_data.data;
    auto targets = (RidgeAggregateState **)target_data.data;
