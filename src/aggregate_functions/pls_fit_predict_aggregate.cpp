...
//===--------------------------------------------------------------------===//
// PLS Predict Aggregate State
//===--------------------------------------------------------------------===//
struct PlsPredictAggState {
    ...
    PlsPredictAggState() : n_features(0), initialized(false), n_components(1), fit_intercept(true) {}
...
static void PlsPredictAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) PlsPredictAggState();
}

static void PlsPredictAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (PlsPredictAggState **)sdata.data;

    for (idx_t i = 0; i < count; i++) {
        auto &state = *states[sdata.sel->get_index(i)];
        state.~PlsPredictAggState();
    }
}

static void PlsPredictAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                                 Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<PlsPredictAggBindData>();
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        state.n_components = bind_data.n_components;
...
static void PlsPredictAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat source_data, target_data;
    source_vector.ToUnifiedFormat(count, source_data);
    target_vector.ToUnifiedFormat(count, target_data);

    auto sources = (PlsPredictAggState **)source_data.data;
    auto targets = (PlsPredictAggState **)target_data.data;
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        if (!target.initialized) {
            ...
            target.n_components = source.n_components;
...
static void PlsPredictAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        options.n_components = state.n_components;
        options.fit_intercept = state.fit_intercept;

        AnofoxPlsFitResultCore core_result;
        AnofoxError error;

        bool success = anofox_pls_fit(y_array, x_arrays.data(), x_arrays.size(), options, &core_result, &error);
        ...
        anofox_free_pls_result(&core_result);
        state.Reset();
    }
}
...
// Bind function
//===--------------------------------------------------------------------===//
static unique_ptr<FunctionData> PlsPredictAggBind(ClientContext &context, AggregateFunction &function,
