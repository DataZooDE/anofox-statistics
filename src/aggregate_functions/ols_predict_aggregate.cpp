...
// Bind Data
//===--------------------------------------------------------------------===//
struct OlsPredictAggBindData : public FunctionData {
    ...
    bool Equals(const FunctionData &other_p) const override {
        ...
        return fit_intercept == other.fit_intercept && confidence_level == other.confidence_level &&
               solver == other.solver && hc_type == other.hc_type &&
...
static void OlsPredictAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) OlsPredictAggState();
}

static void OlsPredictAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
...
static void OlsPredictAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
    ...
    auto &bind_data = aggr_input_data.bind_data->Cast<OlsPredictAggBindData>();
...
static void OlsPredictAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    ...
    auto sources = (OlsPredictAggState **)source_data.data;
    auto targets = (OlsPredictAggState **)target_data.data;
...
static void OlsPredictAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result,
...
void RegisterOlsFitPredictAggregateFunction(ExtensionLoader &loader) {
    // Primary name (new)
    AggregateFunctionSet func_set("ols_fit_predict_agg");
    ...
    d1.examples = {"ols_fit_predict_agg(y, x)"};
    ...
    d2.examples = {"ols_fit_predict_agg(y, x, {'null_policy': 'drop'})"};
    ...
    d3.examples = {"ols_fit_predict_agg(y, x, split_col)"};
    ...
    d4.examples = {"ols_fit_predict_agg(y, x, split_col, {'null_policy': 'drop'})"};
