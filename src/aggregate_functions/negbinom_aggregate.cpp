...
static void NegBinomAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result, idx_t count,
	...
	for (idx_t i = 0; i < count; i++) {
		...
		AnofoxNegBinomialOptions options {}; // value-initialised: these are hand-mirrored C structs, so a
		                                     // field added to the header must not be left as garbage
...
void RegisterNegBinomAggregateFunction(ExtensionLoader &loader) {
	...
	auto map_func = AggregateFunction("anofox_stats_negbinom_fit_agg",
