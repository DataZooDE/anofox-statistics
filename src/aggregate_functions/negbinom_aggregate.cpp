...
//===--------------------------------------------------------------------===//
// Negative Binomial GLM aggregate. Mirrors the Poisson aggregate; the only
// upstream-visible difference is the FFI entry point (anofox_negbinomial_fit)
// and the option set — there is no link selection (log only) and no `lambda`
// regularization knob upstream for NegBin.
//===--------------------------------------------------------------------===//
struct NegBinomAggregateState {
...
struct NegBinomAggregateBindData : public FunctionData {
...
static LogicalType GetNegBinomAggResultType(bool compute_inference) {
...
static void NegBinomAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) NegBinomAggregateState();
}

static void NegBinomAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (NegBinomAggregateState **)sdata.data;

    for (idx_t i = 0; i < count; i++) {
        auto &state = *states[sdata.sel->get_index(i)];
        state.~NegBinomAggregateState();
    }
}

static void NegBinomAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                              Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<NegBinomAggregateBindData>();
...
static void NegBinomAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat source_data, target_data;
    source_vector.ToUnifiedFormat(count, source_data);
    target_vector.ToUnifiedFormat(count, target_data);

    auto sources = (NegBinomAggregateState **)source_data.data;
    auto targets = (NegBinomAggregateState **)target_data.data;
...
static void NegBinomAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result, idx_t count,
                                idx_t offset) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (NegBinomAggregateState **)sdata.data;
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        AnofoxNegBinomialOptions options;
        ...
        bool success = anofox_negbinomial_fit(y_array, x_arrays.data(), x_arrays.size(), options, &core_result,
        ...
        // overdispersion parameter — i.e. var(y) = mu + alpha * mu^2.
...
static unique_ptr<FunctionData> NegBinomAggBind(ClientContext &context, AggregateFunction &function,
...
void RegisterNegBinomAggregateFunction(ExtensionLoader &loader) {
    AggregateFunctionSet func_set("anofox_stats_negbinom_fit_agg");

    auto basic_func = AggregateFunction(
        "anofox_stats_negbinom_fit_agg", {LogicalType::DOUBLE, LogicalType::LIST(LogicalType::DOUBLE)},
    ...
    auto map_func = AggregateFunction(
        ...
        AggregateFunction::StateSize<NegBinomAggregateState>, NegBinomAggInitialize, NegBinomAggUpdate,
        NegBinomAggCombine, NegBinomAggFinalize, nullptr, NegBinomAggBind, NegBinomAggDestroy);
    func_set.AddFunction(map_func);

    CreateAggregateFunctionInfo info(std::move(func_set));
    info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
    FunctionDescription d1;
    d1.description = "Fits a Negative Binomial GLM (log link, overdispersion parameter estimated jointly) and "
                     "returns coefficients, deviance, AIC, dispersion (= alpha), and fit statistics.";
    d1.examples = {"anofox_stats_negbinom_fit_agg(y, x, {'fit_intercept': true})"};
    ...
    d2.description = "Fits a Negative Binomial GLM (log link, overdispersion parameter estimated jointly) and "
                     "returns coefficients, deviance, AIC, dispersion (= alpha), and fit statistics.";
    d2.examples = {"anofox_stats_negbinom_fit_agg(y, x)"};
    ...
    {
        AggregateFunctionSet alias_set("negbinom_fit_agg");
        alias_set.AddFunction(basic_func);
        alias_set.AddFunction(map_func);
        CreateAggregateFunctionInfo alias_info(std::move(alias_set));
        alias_info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
        alias_info.alias_of = "anofox_stats_negbinom_fit_agg";
