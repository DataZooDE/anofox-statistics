...
void RegisterRidgeFitPredictAggregateFunction(ExtensionLoader &loader) {
    // Primary name (new)
    AggregateFunctionSet func_set("ridge_fit_predict_agg");
    ...
    d3.examples = {"ridge_fit_predict_agg(y, x, split_col)"};
    ...
    d4.examples = {"ridge_fit_predict_agg(y, x, split_col, {'null_policy': 'drop'})"};
