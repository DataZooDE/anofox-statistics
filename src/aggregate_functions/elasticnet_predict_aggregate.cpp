...
void RegisterElasticNetFitPredictAggregateFunction(ExtensionLoader &loader) {
    // Primary name (new)
    AggregateFunctionSet func_set("elasticnet_fit_predict_agg");
    ...
    d3.examples = {"elasticnet_fit_predict_agg(y, x, split_col)"};
    ...
    d4.examples = {"elasticnet_fit_predict_agg(y, x, split_col, {'null_policy': 'drop'})"};
