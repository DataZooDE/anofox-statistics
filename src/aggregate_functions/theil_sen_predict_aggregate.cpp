...
struct TheilSenPredictAggState {
...
struct TheilSenPredictAggBindData : public FunctionData {
    ...
    bool Equals(const FunctionData &other_p) const override {
        auto &o = other_p.Cast<TheilSenPredictAggBindData>();
...
static LogicalType GetTheilSenPredictAggResultType() {
...
static void TheilSenPredictAggInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) TheilSenPredictAggState();
}

static void TheilSenPredictAggDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (TheilSenPredictAggState **)sdata.data;

    for (idx_t i = 0; i < count; i++) {
        auto &state = *states[sdata.sel->get_index(i)];
        state.~TheilSenPredictAggState();
    }
}
...
static void TheilSenPredictAggUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                                     Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<TheilSenPredictAggBindData>();
...
static void TheilSenPredictAggCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &,
    ...
    auto sources = (TheilSenPredictAggState **)source_data.data;
    auto targets = (TheilSenPredictAggState **)target_data.data;
...
static void TheilSenPredictAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result,
...
static unique_ptr<FunctionData> TheilSenPredictAggBind(ClientContext &context, AggregateFunction &function,
...
static unique_ptr<FunctionData> TheilSenPredictAggBindWithSplit(ClientContext &context,
