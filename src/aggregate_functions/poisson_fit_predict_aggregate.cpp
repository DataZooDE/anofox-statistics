...
struct PoissonFitPredictAggState {
	...
	PoissonFitPredictAggState()
	    : n_features(0), initialized(false), fit_intercept(true), link(ANOFOX_POISSON_LINK_LOG), max_iterations(100),
	      tolerance(1e-8), confidence_level(0.95), glm_lambda(0.0), null_policy(NullPolicy::DROP) {
	}
...
struct PoissonFitPredictAggBindData : public FunctionData {
	...
	bool Equals(const FunctionData &other_p) const override {
		...
		return fit_intercept == other.fit_intercept && link == other.link && max_iterations == other.max_iterations &&
		       ...
		       glm_lambda == other.glm_lambda && null_policy == other.null_policy &&
...
static void PoissonFitPredictAggFinalize(Vector &state_vector, AggregateInputData &aggr_input_data, Vector &result,
	...
	for (idx_t i = 0; i < count; i++) {
		...
		AnofoxPoissonOptions options {}; // value-initialised: these are hand-mirrored C structs, so a
