...
struct RidgeFitPredictBindData : public FunctionData {
...
static LogicalType GetRidgeFitPredictResultType() {
...
static void RidgeFitPredictInitialize(const AggregateFunction &, data_ptr_t state_p) {
    new (state_p) RidgeFitPredictState();
}

static void RidgeFitPredictDestroy(Vector &state_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (RidgeFitPredictState **)sdata.data;
    for (idx_t i = 0; i < count; i++) {
        auto &state = *states[sdata.sel->get_index(i)];
        state.~RidgeFitPredictState();
    }
}

static void RidgeFitPredictUpdate(Vector inputs[], AggregateInputData &aggr_input_data, idx_t input_count,
                                   Vector &state_vector, idx_t count) {
    auto &bind_data = aggr_input_data.bind_data->Cast<RidgeFitPredictBindData>();
...
static void RidgeFitPredictCombine(Vector &source_vector, Vector &target_vector, AggregateInputData &, idx_t count) {
    UnifiedVectorFormat source_data, target_data;
    source_vector.ToUnifiedFormat(count, source_data);
    target_vector.ToUnifiedFormat(count, target_data);

    auto sources = (RidgeFitPredictState **)source_data.data;
    auto targets = (RidgeFitPredictState **)target_data.data;
...
static void RidgeFitPredictFinalize(Vector &state_vector, AggregateInputData &, Vector &result, idx_t count,
                                     idx_t offset) {
    UnifiedVectorFormat sdata;
    state_vector.ToUnifiedFormat(count, sdata);
    auto states = (RidgeFitPredictState **)sdata.data;
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        bool success =
            anofox_ridge_fit(y_array, x_arrays.data(), x_arrays.size(), options, &core_result, nullptr, &error);
...
static unique_ptr<FunctionData> RidgeFitPredictBind(ClientContext &context, AggregateFunction &function,
